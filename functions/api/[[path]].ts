...
const actionFromParams = (path: string | string[] | undefined) =>
  Array.isArray(path) ? path[path.length - 1] : String(path || "").split("/").filter(Boolean).pop();
...
const fetchUrl = async (req: Request) => {
  ...
  const response = await fetch(url, {
    headers: {
      "user-agent": "Mozilla/5.0 (Yiyan Cloudflare Pages Function)",
...
const translateYoudao = async (req: Request, env: Env) => {
  const body = await readBody(req);
  const text = body.text || "";
  const appKey = env.YOUDAO_APP_KEY || body.appKey || "";
  const appSecret = env.YOUDAO_APP_SECRET || "";

  if (!text.trim()) return json({ ok: false, error: "没有要翻译的文本" }, 400);
  if (!appKey || !appSecret) return json({ ok: false, error: "Cloudflare 还没有配置有道环境变量" }, 500);
...
export const onRequest = async (context: PagesContext) => {
  ...
  const action = actionFromParams(params.path);
  try {
    if (action === "health") return json({ ok: true, service: "译言 Cloudflare 代理" });
    if (request.method !== "POST") return json({ ok: false, error: "Method not allowed" }, 405);
    if (action === "fetch-url") return await fetchUrl(request);
    if (action === "youdao") return await translateYoudao(request, env);
